...
class openqaPromoptGeneration():
    def __init__(self, api_key):
        self.api_key = api_key
        self.hidden_instruct = 'openqa_prompt_generation'
    ...
    def requests_to_chat(self, prompt_input, text, iter=5):
        ...
        print(data_sets)       
        with open(os.path.join('./openqa_prompt_db/','openqa_prompt_generation_'+now_str+'.jsonl'), 'w', encoding='utf8') as fw:
            for data in data_sets:
                data['content'] = text
                fw.write(json.dumps(data, ensure_ascii=False)+'\n')
                
        return {'save_path': os.path.join('./openqa_prompt_db/','openqa_prompt_generation_'+now_str+'.jsonl')}
